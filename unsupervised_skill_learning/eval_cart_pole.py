"""Evaluate DADS skills on the CartPole environment.

Mirrors eval_humulum.py but for cart-pole swing-up.
Height metric: pole tip z = xpos['pole','z'] - xmat['pole','zz']
This matches val/scripts/cip/plots/plot_combined_height.py cart_pole_tip_heights.

Usage:
    python unsupervised_skill_learning/eval_cart_pole.py \\
...
flags.DEFINE_string('environment', 'CartPole', 'Environment name (must match training)')
...
flags.DEFINE_integer('expose_last_action', 1, 'Unused for CartPole')
flags.DEFINE_integer('expose_upright', 1, 'Unused for CartPole')
flags.DEFINE_float('upright_threshold', 0.9, 'Unused for CartPole')
flags.DEFINE_float('robot_noise_ratio', 0.05, 'Unused for CartPole')
flags.DEFINE_float('root_noise_ratio', 0.002, 'Unused for CartPole')
flags.DEFINE_float('scale_root_position', 1, 'Unused for CartPole')
flags.DEFINE_integer('run_on_hardware', 0, 'Unused for CartPole')
flags.DEFINE_float('randomize_hfield', 0.0, 'Unused for CartPole')
flags.DEFINE_integer('observation_omission_size', 0, 'Unused for CartPole')
flags.DEFINE_integer('randomized_initial_distribution', 0, 'Unused for CartPole')
flags.DEFINE_float('horizontal_wrist_constraint', 1.0, 'Unused for CartPole')
flags.DEFINE_float('vertical_wrist_constraint', 1.0, 'Unused for CartPole')
...
def _build_eval_env(preset_skill, inner_env=None):
    if inner_env is None:
        inner_env = cart_pole_env.CartPoleEnv()
...
def _run_episode(py_env, policy, inner_env):
    """Run one episode; return array of pole-tip heights at each step."""
    time_step = py_env.reset()
    heights = []
    while not time_step.is_last():
        heights.append(inner_env.pole_tip_height())
...
def main(_):
    ...
    dummy_inner = cart_pole_env.CartPoleEnv()
    ...
    with tf.compat.v1.Session().as_default() as sess:
        ...
        fig.savefig(os.path.join(out_dir, 'pole_tip_height.png'), dpi=150)
        np.save(os.path.join(out_dir, 'pole_tip_height.npy'), best_heights)
        print(f'\nSaved pole-tip-height plot → {out_dir}/pole_tip_height.png')
        plt.close(fig)
        ...
        ax.set_title(f'DADS CartPole step {step} — best skill {best_idx} '
                     f'({FLAGS.num_seeds} stochastic seeds)')
        ax.set_xlabel('Step')
        ax.set_ylabel('Pole tip height (m)')
