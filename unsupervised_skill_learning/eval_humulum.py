"""Evaluate DADS skills on the Humulum environment.

Reproduces the analysis of track_head_height.py and plot_best_skill_seeds.py
from the url_benchmark for a trained DADS checkpoint.

Usage:
    python unsupervised_skill_learning/eval_humulum.py \\
...
flags.DEFINE_integer('expose_last_action', 1, 'Unused for Humulum')
flags.DEFINE_integer('expose_upright', 1, 'Unused for Humulum')
flags.DEFINE_float('upright_threshold', 0.9, 'Unused for Humulum')
flags.DEFINE_float('robot_noise_ratio', 0.05, 'Unused for Humulum')
flags.DEFINE_float('root_noise_ratio', 0.002, 'Unused for Humulum')
flags.DEFINE_float('scale_root_position', 1, 'Unused for Humulum')
flags.DEFINE_integer('run_on_hardware', 0, 'Unused for Humulum')
flags.DEFINE_float('randomize_hfield', 0.0, 'Unused for Humulum')
flags.DEFINE_integer('observation_omission_size', 0, 'Unused for Humulum')
flags.DEFINE_integer('randomized_initial_distribution', 0, 'Unused for Humulum')
flags.DEFINE_float('horizontal_wrist_constraint', 1.0, 'Unused for Humulum')
flags.DEFINE_float('vertical_wrist_constraint', 1.0, 'Unused for Humulum')
...
def _build_eval_env(preset_skill, inner_env=None):
    """Build a wrapped eval environment with a fixed preset skill.

    If inner_env is provided it is reused (shares the same mujoco sim), which
    lets the caller read head_height() without double-stepping.
    """
    if inner_env is None:
        inner_env = humulum_env.HumulumEnv()
...
def _run_episode(py_env, policy, inner_env):
    """Run a single episode; return array of head heights at each step."""
    time_step = py_env.reset()
    heights = []
    while not time_step.is_last():
        heights.append(inner_env.head_height())
...
def main(_):
    ...
    # ── build graph ──────────────────────────────────────────────────────────
    # A dummy env is needed to infer specs before the session is created.
    dummy_inner = humulum_env.HumulumEnv()
    ...
    with tf.compat.v1.Session().as_default() as sess:
        ...
        # ── sample eval skills from the cont_uniform prior ────────────────────
        ...
        # ── track_head_height: deterministic episode per skill ────────────────
        ...
        fig.suptitle(f'DADS Humulum — step {step}')
        ...
        hh_png = os.path.join(out_dir, 'head_height.png')
        hh_npy = os.path.join(out_dir, 'head_height.npy')
        fig.savefig(hh_png, dpi=150)
        np.save(hh_npy, best_heights)
        print(f'\nSaved head-height plot → {hh_png}')
        print(f'Saved best-skill trajectory → {hh_npy}')
        plt.close(fig)
        ...
        # ── plot_best_skill_seeds: stochastic runs of the best skill ──────────
        ...
        ax.set_title(f'DADS Humulum step {step} — best skill {best_idx} '
        ...
        bs_png = os.path.join(out_dir, 'best_skill_seeds.png')
        bs_npy = os.path.join(out_dir, 'best_skill_seeds.npy')
        fig.savefig(bs_png, dpi=150)
        np.save(bs_npy, arr)
        print(f'\nSaved best-skill-seeds plot → {bs_png}')
