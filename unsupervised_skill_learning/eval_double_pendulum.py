"""Evaluate DADS skills on the DoublePendulum environment.

Height metric: geom_xpos['bob2','z']  (matches url_benchmark and val).
val formula: (xpos['second_link'] + R @ [0,0,-1])[z] = geom_xpos['bob2','z'] ✓

Usage:
    python unsupervised_skill_learning/eval_double_pendulum.py \\
        --logdir=/path/to/logdir \\
        --flagfile=configs/double_pendulum_offpolicy.txt \\
        [--num_eval_skills=20] \\
        [--num_seeds=8] \\
        [--out_dir=./eval_double_pendulum]
"""
...
from envs import double_pendulum as dp_env
from lib import py_tf_policy

FLAGS = flags.FLAGS

flags.DEFINE_string('logdir', '~/tmp/dads', 'Directory where training data was saved')
flags.DEFINE_string('environment', 'DoublePendulum', 'Environment name (must match training)')
...
flags.DEFINE_integer('expose_last_action', 1, 'Unused for DoublePendulum')
flags.DEFINE_integer('expose_upright', 1, 'Unused for DoublePendulum')
flags.DEFINE_float('upright_threshold', 0.9, 'Unused for DoublePendulum')
flags.DEFINE_float('robot_noise_ratio', 0.05, 'Unused for DoublePendulum')
flags.DEFINE_float('root_noise_ratio', 0.002, 'Unused for DoublePendulum')
flags.DEFINE_float('scale_root_position', 1, 'Unused for DoublePendulum')
flags.DEFINE_integer('run_on_hardware', 0, 'Unused for DoublePendulum')
flags.DEFINE_float('randomize_hfield', 0.0, 'Unused for DoublePendulum')
flags.DEFINE_integer('observation_omission_size', 0, 'Unused for DoublePendulum')
flags.DEFINE_integer('randomized_initial_distribution', 0, 'Unused for DoublePendulum')
flags.DEFINE_float('horizontal_wrist_constraint', 1.0, 'Unused for DoublePendulum')
flags.DEFINE_float('vertical_wrist_constraint', 1.0, 'Unused for DoublePendulum')
...
flags.DEFINE_string('out_dir', './eval_double_pendulum', 'Directory to save output plots and npy files')
...
def _build_eval_env(preset_skill, inner_env=None):
    if inner_env is None:
        inner_env = dp_env.DoublePendulumEnv()
...
def _run_episode(py_env, policy, inner_env):
    """Run one episode; return array of bob2 tip heights at each step."""
...
def main(_):
    ...
    dummy_inner = dp_env.DoublePendulumEnv()
